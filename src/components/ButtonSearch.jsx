...
const Button = ({ children, variant, className, width, height, onClick }) => {
  ...
  return (
    <button 
      style={{ width, height }}  // Setting the width and height from props
      className={`${styles[variant]} ${className} text-xs leading-5`} // Applying variant-specific styles and additional className
      onClick={onClick} // onClick handler from props
    >
      {children} 
    </button>
  );
