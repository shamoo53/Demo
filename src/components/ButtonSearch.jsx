...
const Button = ({ children, variant, className, width, height, onClick }) => {
  const baseStyles = 'flex items-center justify-center rounded-md text-sm font-semibold';
  const styles = {
    primary: `${baseStyles} bg-[#413FA0] text-white hover:bg-[#3a36a0]`,
    secondary: `${baseStyles} border border-gray-300 text-gray-700 bg-white hover:bg-gray-50`,
  };
...
const ButtonSearch = ({ openModal }) => {
  return (
    <div className="flex items-center justify-between px-4 py-1 bg-[#F2F2F2] text-xs leading-5">
      <div className="flex space-x-4" style={{ marginLeft: '-15px' }}
